...
| <code><var>error</var>WarnOnlyPatterns</code> | Reports the specified `<var>error</var>` as a warning rather than a failure. Value is either `hashNotFound` or `http<var>XXX</var>`, as follows:<ul><li>`hashNotFoundWarnOnlyPatterns`. Reports `404` errors as warnings.</li><li>`http<var>XXX</var>WarnOnlyPatterns`. Reports the specified `<var>XXX</var>` errors as warnings. The `<var>XXX</var>` value is a number from `400` to `510`.</li></ul> |
