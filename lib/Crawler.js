...
function setConfigRootUrl(config, crawlerSeedUrl) {
  ...
  crawlerConfig.rootUrl = `${protocolString}://${crawlerSeedUrl.hostname}${portString}`;
...
function checkTitle(url, pattern, $, errorHandler) {
  $('title').each(function handleTitle() {
  ...
  crawler.discoverResources = function discoverResources(buf, queueItem) {
    ...
    if (queueItem.stateData.contentType.indexOf('text/html') === 0) {
      ...
      if (queueItem.host === crawler.host) {
        ...
        $('a[href]').each(function registerHash() {
...
module.exports = function spidersuiteCrawler(myUrl, config, verbose) {
  return {
    crawl() {
      const crawler = new Crawler(myUrl);
