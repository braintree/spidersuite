...
const CHAR_INFINITY = '∞';
const CHAR_DISALLOWED = '≯';

const MAX_LINKS_FROM = 5;
const MAX_REDIRECTS_FROM = 5;
...
module.exports = function Logger(crawler, verbose) {
  if (verbose) {
    cursor
      .reset()
      .write('\n\n========================\nConfiguration\n========================\n')
  ...
  return {
    ...
    /**
     * Log a message letting the user know a URL was disallowed by the crawler.
     * @param {URL} url the URL that was disallowed
     */
    logDisallowed(url) {
      if (verbose && this.disallowed.indexOf(url) === -1) {
        this.disallowed.push(url);
        clearLine();
        cursor
          .grey()
          .write(`${CHAR_DISALLOWED} ${url}`)
          .reset()
          .write('\n');
      }
    },
    ...
    logRedirect(queueItem, parsedURL) {
      ...
      if (!parsedURL) {
        ...
        cursor
          ...
          .write(' ')
          .write('redirect invalid and/or to another domain, and halted by simplecrawler.')
          .reset()
          .write('\n');

        this.haltedRedirects.push({ url: queueItem.url });
      } else {
        url = parsedURL.url;
    ...
    report() {
      ...
      function attachDetail(item) {
        ...
        // Find all links to this URL from other URLs.
        // Hash link errors set their own hashLinkFrom for this purpose.
      ...
      this.haltedRedirects = _.map(this.haltedRedirects, attachDetail);
      ...
      this.haltedRedirects = _.sortBy(this.haltedRedirects, 'url');
      this.ignored = this.ignored.sort();
      this.disallowed = this.disallowed.sort();
      ...
      if (verbose) {
        ...
        cursor
          .write('ignored: ')
          .write(JSON.stringify(this.ignored))
          .write('\n');
      ...
      if (this.haltedRedirects.length > 0) {
        cursor
          .write('haltedRedirects: ')
          .write(JSON.stringify(this.haltedRedirects, null, '  '))
          .write('\n');
      }
      if (this.disallowed.length > 0) {
        cursor
          .write('disallowed: ')
          .write(JSON.stringify(this.disallowed, null, '  '))
          .write('\n');
      }
