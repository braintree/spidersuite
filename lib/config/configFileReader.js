...
const defaultConfig = require(path.resolve('./defaultConfig'));

const debug = require('debug')('spider:configFileReader');
...
function applyExtends(config, filePath, relativeTo) {
  ...
  extendedConfig = configExtends.reduceRight((previousValue, parentPath) => {
    try {
      let loadedConfig;

      if (parentPath === 'spider:default') {
        ...
        loadedConfig = load(parentPath);
      } else {
        throw new Error(`extends=${parentPath} not supported.`);
      }
